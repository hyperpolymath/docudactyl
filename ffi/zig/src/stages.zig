...
// Investigative document analysis stages (bits 20-23)
pub const STAGE_REDACTION_DETECT: u64 = 1 << 20;
pub const STAGE_FINANCIAL_EXTRACT: u64 = 1 << 21;
pub const STAGE_LEGAL_NER: u64 = 1 << 22;
pub const STAGE_SPEAKER_ID: u64 = 1 << 23;

/// Total number of defined stages.
pub const STAGE_COUNT: u6 = 24;
...
/// Investigative stages — redaction, financial, legal, speaker ID.
pub const STAGE_INVESTIGATIVE: u64 = STAGE_ANALYSIS | STAGE_NER |
    STAGE_REDACTION_DETECT | STAGE_FINANCIAL_EXTRACT |
    STAGE_LEGAL_NER | STAGE_SPEAKER_ID | STAGE_LAYOUT_ANALYSIS |
    STAGE_HANDWRITING_OCR;
...
/// Redaction detection — scans PDF content streams for overlay rectangles
/// that mask underlying text (common in legal document releases like Epstein files).
/// Detects: black fill rectangles, annotation-based redactions, recoverable text
/// under overlay-only redactions where the content stream was not scrubbed.
fn stageRedactionDetect(b: *capnp.Builder, input_path: [*:0]const u8) void {
    // Open with Poppler to inspect page annotations and content streams.
    const uri_buf = uriFromPath(input_path) orelse {
        b.setText(capnp.PTR_REDACT_STATUS, "error");
        b.setText(capnp.PTR_REDACT_REASON, "Could not construct URI from input path");
        return;
    };
    const doc = c.poppler_document_new_from_file(&uri_buf, null, null) orelse {
        b.setText(capnp.PTR_REDACT_STATUS, "not_applicable");
        b.setText(capnp.PTR_REDACT_REASON, "Not a PDF or could not open");
        return;
    };

    const n_pages = c.poppler_document_get_n_pages(doc);
    var redaction_count: u32 = 0;
    var recoverable_count: u32 = 0;

    var page_idx: c_int = 0;
    while (page_idx < n_pages) : (page_idx += 1) {
        const page = c.poppler_document_get_page(doc, page_idx) orelse continue;

        // Check for redaction annotations (Subtype /Redact)
        ...
        while (item) |node| : (item = node.*.next) {
            ...
            // Type 12 = POPPLER_ANNOT_REDACT, Type 1 = POPPLER_ANNOT_HIGHLIGHT (sometimes used)
            if (annot_type == 12) {
                redaction_count += 1;
            }
        }
        ...
        // Check if text is extractable under redacted areas (recoverable redaction)
        const text_ptr = c.poppler_page_get_text(page);
        if (text_ptr) |txt| {
            const text = std.mem.span(txt);
            // Heuristic: if page has redaction annotations but also has extractable text,
            // the redaction may be overlay-only (text not actually removed from stream)
            if (redaction_count > 0 and text.len > 0) {
                recoverable_count += 1;
            }
            c.g_free(txt);
        }
    ...
    const summary = std.fmt.bufPrint(&summary_buf, "{d} redaction(s) detected across {d} pages, {d} potentially recoverable", .{
        redaction_count,
        @as(u32, @intCast(n_pages)),
        recoverable_count,
    }) catch "Redaction detection complete";

    b.setText(capnp.PTR_REDACT_STATUS, if (redaction_count > 0) "redacted" else "clean");
    b.setText(capnp.PTR_REDACT_REASON, summary);
    b.setU32(capnp.OFF_REDACT_COUNT, redaction_count);
}

/// Financial entity extraction — pattern-based detection of monetary amounts,
/// account numbers, transaction references, and currency codes in extracted text.
fn stageFinancialExtract(b: *capnp.Builder, text: []const u8) void {
    var amount_count: u32 = 0;
    var account_count: u32 = 0;
    var i: usize = 0;

    while (i < text.len) : (i += 1) {
        // Detect currency symbols/codes followed by digits: $, £, €, USD, GBP, EUR
        if (text[i] == '$' or text[i] == 0xC2 or // £ = C2 A3 in UTF-8
            text[i] == 0xE2) // € = E2 82 AC in UTF-8
        {
            // Scan forward for digits
            var j = i + 1;
            if (text[i] == 0xC2 or text[i] == 0xE2) j = i + 2; // skip multi-byte
            if (j < text.len and text[i] == 0xE2) j = i + 3;
            while (j < text.len and (text[j] == ' ' or text[j] == ',')) : (j += 1) {}
            if (j < text.len and text[j] >= '0' and text[j] <= '9') {
                amount_count += 1;
            }
        }

        // Detect "USD", "GBP", "EUR", "CHF" followed by space and digits
        if (i + 4 < text.len and text[i + 3] == ' ') {
            const code = text[i .. i + 3];
            if (std.mem.eql(u8, code, "USD") or std.mem.eql(u8, code, "GBP") or
                std.mem.eql(u8, code, "EUR") or std.mem.eql(u8, code, "CHF") or
                std.mem.eql(u8, code, "JPY") or std.mem.eql(u8, code, "CAD"))
            {
                if (i + 4 < text.len and text[i + 4] >= '0' and text[i + 4] <= '9') {
                    amount_count += 1;
                }
            }
        }

        // Detect account-like patterns: sequences of 8+ digits with optional dashes
        if (text[i] >= '0' and text[i] <= '9') {
            var digit_count: u32 = 0;
            var k = i;
            while (k < text.len and (text[k] >= '0' and text[k] <= '9' or text[k] == '-')) : (k += 1) {
                if (text[k] >= '0' and text[k] <= '9') digit_count += 1;
            }
            if (digit_count >= 8 and digit_count <= 20) {
                account_count += 1;
                i = k; // skip past
            }
        }
    ...
    var summary_buf: [256]u8 = undefined;
    const summary = std.fmt.bufPrint(&summary_buf, "{d} monetary amount(s), {d} account-like pattern(s)", .{
        amount_count,
        account_count,
    }) catch "Financial extraction complete";

    b.setText(capnp.PTR_FINANCIAL_STATUS, if (amount_count > 0 or account_count > 0) "entities_found" else "none_found");
    b.setText(capnp.PTR_FINANCIAL_REASON, summary);
    b.setU32(capnp.OFF_FINANCIAL_AMOUNTS, amount_count);
    b.setU32(capnp.OFF_FINANCIAL_ACCOUNTS, account_count);
}

/// Legal NER — pattern-based detection of legal entities: case citations,
/// docket numbers, statute references, judge names, and party identifiers.
/// Complements the general NER stage with domain-specific legal patterns.
fn stageLegalNer(b: *capnp.Builder, text: []const u8) void {
    var case_citations: u32 = 0;
    var docket_refs: u32 = 0;
    var statute_refs: u32 = 0;
    var i: usize = 0;

    while (i < text.len) : (i += 1) {
        // Case citations: "v." or "vs." pattern (e.g. "Doe v. Epstein")
        if (i + 2 < text.len and text[i] == 'v' and text[i + 1] == '.') {
            // Check preceding word and following word exist
            if (i > 0 and text[i - 1] == ' ' and i + 2 < text.len and text[i + 2] == ' ') {
                case_citations += 1;
            }
        }

        // Docket numbers: patterns like "No. 08-cv-1234" or "Case 1:15-cv-07433"
        if (i + 3 < text.len) {
            const prefix = text[i .. i + 3];
            if (std.mem.eql(u8, prefix, "No.") or std.mem.eql(u8, prefix, "no.")) {
                docket_refs += 1;
            }
        }
        if (i + 4 < text.len) {
            const prefix = text[i .. i + 4];
            if (std.mem.eql(u8, prefix, "Case") and i + 5 < text.len and text[i + 4] == ' ') {
                // Look for digit following "Case "
                if (text[i + 5] >= '0' and text[i + 5] <= '9') {
                    docket_refs += 1;
                }
            }
        }

        // Statute references: "U.S.C." or "§" (section symbol, C2 A7 in UTF-8)
        if (i + 5 < text.len and std.mem.eql(u8, text[i .. i + 5], "U.S.C")) {
            statute_refs += 1;
        }
    ...
    const total = case_citations + docket_refs + statute_refs;
    var summary_buf: [256]u8 = undefined;
    const summary = std.fmt.bufPrint(&summary_buf, "{d} case citation(s), {d} docket ref(s), {d} statute ref(s)", .{
        case_citations,
        docket_refs,
        statute_refs,
    }) catch "Legal NER complete";

    b.setText(capnp.PTR_LEGAL_STATUS, if (total > 0) "entities_found" else "none_found");
    b.setText(capnp.PTR_LEGAL_REASON, summary);
    b.setU32(capnp.OFF_LEGAL_CASES, case_citations);
    b.setU32(capnp.OFF_LEGAL_DOCKETS, docket_refs);
    b.setU32(capnp.OFF_LEGAL_STATUTES, statute_refs);
}

/// URI helper for Poppler (expects file:// URIs).
fn uriFromPath(path: [*:0]const u8) ?[4096]u8 {
    const path_slice = std.mem.span(path);
    const prefix = "file://";
    var buf: [4096]u8 = undefined;
    if (prefix.len + path_slice.len >= 4096) return null;
    @memcpy(buf[0..prefix.len], prefix);
    @memcpy(buf[prefix.len .. prefix.len + path_slice.len], path_slice);
    buf[prefix.len + path_slice.len] = 0;
    return buf;
}
...
pub fn runStages(ctx: StageContext) void {
    ...
    const needs_text = (ctx.stages & (STAGE_LANGUAGE_DETECT | STAGE_READABILITY |
        STAGE_KEYWORDS | STAGE_CITATION_EXTRACT |
        STAGE_FINANCIAL_EXTRACT | STAGE_LEGAL_NER)) != 0;
    ...
    if (needs_text) {
        ...
        if (readExtractedText(ctx.output_path, arena.allocator())) |text| {
            ...
            if (ctx.stages & STAGE_FINANCIAL_EXTRACT != 0)
                stageFinancialExtract(&b, text);

            if (ctx.stages & STAGE_LEGAL_NER != 0)
                stageLegalNer(&b, text);
        }
    }
    ...
    // ── Phase 9: Investigative document analysis stages ───────────────

    if (ctx.stages & STAGE_REDACTION_DETECT != 0 and ctx.content_kind == CK_PDF) {
        stageRedactionDetect(&b, ctx.input_path);
    }

    if (ctx.stages & STAGE_SPEAKER_ID != 0) {
        // Speaker ID dispatches to ML — uses stage_id 5 for speaker diarization model
        if (ctx.ml_handle) |ml| {
            var ml_result: ml_inference.MlResult = std.mem.zeroes(ml_inference.MlResult);
            const rc = ml_inference.ddac_ml_run_stage(ml, 5, ctx.input_path, &ml_result);
            if (rc == 0 and ml_result.status == 0) {
                var spk_buf: [256]u8 = undefined;
                const spk_summary = std.fmt.bufPrint(&spk_buf, "SpeakerID: {d} speaker(s), conf={d:.2}, {d}us", .{
                    ml_result.output_count,
                    ml_result.confidence,
                    ml_result.inference_time_us,
                }) catch "SpeakerID";
                b.setText(capnp.PTR_SPEAKER_STATUS, "ok");
                b.setText(capnp.PTR_SPEAKER_REASON, spk_summary);
                b.setU32(capnp.OFF_SPEAKER_COUNT, @intCast(ml_result.output_count));
            } else {
                b.setText(capnp.PTR_SPEAKER_STATUS, "model_not_found");
                b.setText(capnp.PTR_SPEAKER_REASON, "Requires speaker diarization model. Place speaker_id.onnx in modelDir.");
            }
        } else {
            writeStub(&b, capnp.PTR_SPEAKER_STATUS, capnp.PTR_SPEAKER_REASON, "Requires ML runtime for speaker diarization.");
        }
    }
